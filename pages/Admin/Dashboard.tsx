...
import { 
  ...
  Mail, Palette, UserCircle, Image as ImageIcon
} from 'lucide-react';
...
const AdminDashboard = () => {
  ...
  const menuItems = [
    { id: 'siteConfig', label: 'Home & Styling', icon: Palette },
  ...
  const getFormFields = () => {
    switch(activeTab) {
      case 'siteConfig':
        return [
          { name: 'heroTitle', label: 'Hero Headline', type: 'text' },
          { name: 'heroSubTitle', label: 'Hero Subtitle', type: 'textarea' },
          { name: 'marqueeUpdates', label: 'Marquee News (Comma sep)', type: 'text' },
          { name: 'logoUrl', label: 'Logo URL (Image Link)', type: 'text' },
          { name: 'primaryColor', label: 'Primary Brand Color (Hex: #2563eb)', type: 'text' }
        ];
      case 'profile':
        return [
    ...
    <div className="min-h-screen bg-gray-50 flex">
      <aside className="w-64 bg-slate-900 text-white flex flex-col fixed inset-y-0 shadow-2xl z-20">
        <div className="p-6 border-b border-slate-800 flex items-center gap-2">
          ...
          <span className="font-bold text-lg">Cloud Admin</span>
        </div>
      ...
      <main className="flex-1 ml-64 p-8">
        <div className="flex justify-between items-center mb-10">
          <div>
            ...
            <p className="text-slate-400 text-sm">Real-time control via Firebase Cloud.</p>
          </div>
          ...
          <div className="bg-white rounded-3xl shadow-sm border border-gray-100 overflow-hidden">
            {activeTab === 'contacts' ? (
              <div className="p-4 space-y-4">
                 ...
                 {data.length === 0 && <p className="text-center py-10 text-gray-400">No contact messages received yet.</p>}
              </div>
            ) : activeTab === 'siteConfig' || activeTab === 'profile' ? (
              <div className="p-8">
                <button onClick={() => { setEditingItem(data[0]); setShowModal(true); }} className="w-full py-4 bg-slate-900 text-white rounded-2xl font-black">Edit {activeTab}</button>
              </div>
            ) : (
                  ...
                  {data.map((item) => (
                    <tr key={item.id} className="hover:bg-slate-50 transition">
                      <td className="px-8 py-5">
                        ...
                        <div className="text-xs text-slate-400 font-bold uppercase mt-1">{item.company || item.subject || item.branch}</div>
                      </td>
                      <td className="px-8 py-5 text-right flex justify-end gap-3">
                        <button onClick={() => { setEditingItem(item); setShowModal(true); }} className="p-2.5 text-blue-600 hover:bg-blue-50 rounded-xl"><Edit size={20}/></button>
                        <button onClick={() => handleDelete(item.id)} className="p-2.5 text-red-600 hover:bg-red-50 rounded-xl"><Trash2 size={20}/></button>
                      </td>
                    </tr>
                  ))}
                </tbody>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-md animate-in fade-in">
          <form onSubmit={handleSave} className="bg-white w-full max-w-2xl rounded-[2.5rem] shadow-2xl overflow-hidden animate-in zoom-in-95">
            <div className="px-8 py-6 border-b flex justify-between items-center bg-slate-50">
              <h2 className="text-xl font-black text-slate-900">Manage {activeTab}</h2>
              <button type="button" onClick={() => setShowModal(false)} className="w-10 h-10 bg-white rounded-full flex items-center justify-center shadow-sm"><X size={20}/></button>
            </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {getFormFields().map((field) => (
                  <div key={field.name} className={field.type === 'textarea' || field.name === 'heroTitle' || field.name === 'marqueeUpdates' ? 'md:col-span-2' : ''}>
                    <label className="block text-[10px] font-black text-slate-400 uppercase mb-2">{field.label}</label>
                    {field.type === 'textarea' ? (
                      <textarea name={field.name} defaultValue={editingItem?.[field.name]} required rows={3} className="w-full bg-slate-50 border-2 border-transparent rounded-2xl p-4 outline-none focus:border-blue-500 transition-all font-medium" />
                    ) : (
                      <input name={field.name} defaultValue={field.name === 'marqueeUpdates' && Array.isArray(editingItem?.[field.name]) ? editingItem[field.name].join(', ') : editingItem?.[field.name]} required className="w-full bg-slate-50 border-2 border-transparent rounded-2xl p-4 outline-none focus:border-blue-500 transition-all font-bold" />
                    )}
                  </div>
                ))}
              </div>
            ...
            <div className="px-8 py-6 bg-slate-50 border-t flex justify-end gap-4">
              <button type="submit" className="px-10 py-3 bg-blue-600 text-white text-sm font-black rounded-2xl shadow-xl hover:bg-blue-700 transition-all flex items-center gap-2"><Save size={18}/> Update Database</button>
            </div>
          </form>
        </div>
      )}
